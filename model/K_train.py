...
from dataloader import  get_mnist_loaders,myDataset
...
if __name__ == '__main__':
    ...
    mfccs_, melspec_, labels_ = get_mnist_loaders(
        args.data_aug, args.batch_size, args.batch_size,use_k=True
    )
    
    kf=ShuffleSplit(n_splits=3, test_size=0.2)
    
    optimizer = torch.optim.Adam(model.parameters(), lr=args.lr,  weight_decay=args.weight_decay)
    
    num=0
    for train,test in kf.split(mfccs_):
        print(train,test)
        logger.info(f"开始第{format(num+1)}折训练")
        num+=1
        mfccs=mfccs_[train]
        melspec=melspec_[train]
        labels=labels_[train]
        mfccs_test=mfccs_[test]
        melspec_test=melspec_[test]
        labels_test=labels_[test]
            
        train_loader = DataLoader(
            myDataset(mfccs, melspec, labels), batch_size=args.batch_size,
        ...
        score_list.append(best_socre)
        
logger.info("K-Fold最终结果分数为:",np.mean(score_list))

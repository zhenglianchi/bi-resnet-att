...
class ARSC_NET(nn.Module):
    def __init__(self):
        ...
        self.conv1=conv7x7(1,64,7//2)
        self.conv2=conv7x7(1,64,7//2)
        self.bn1=norm(64)
        self.bn2=norm(64)
        self.relu1=nn.ReLU()
        self.relu2=nn.ReLU()
        self.maxpool1=nn.AdaptiveMaxPool2d((64,64))
        self.maxpool2=nn.AdaptiveMaxPool2d((64,64))
        self.resattention1=Residual_attention(64,64)
        self.resattention2=Residual_attention(64,64)
        self.attention=Channel_Spatial_Attention(128)
        self.resattention3=Residual_attention(128,64)
    ...
    def forward(self, x1 , x2):
        ...
        x1=self.relu1(x1)
        x1=self.maxpool1(x1)
        x1=self.resattention1(x1)

        x2=self.conv2(x2)
        x2=self.bn2(x2)
        x2=self.relu2(x2)
        x2=self.maxpool2(x2)
        x2=self.resattention2(x2)

        out=torch.cat([x1, x2], dim=1)
        out=self.attention(out)
        out=self.resattention3(out)
